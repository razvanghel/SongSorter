...
class MyDataRow {
  ...
  bool get _debugInteractive => onSelectChanged != null || cells.any((MyDataCell cell) => cell._debugInteractive);
}

/// The data for a cell of a [DataTable].
///
/// One list of [MyDataCell] objects must be provided for each [MyDataRow]
/// in the [DataTable], in the new [MyDataRow] constructor's `cells`
/// argument.
@immutable
class MyDataCell {
...
class MyDataTable extends StatelessWidget {
  ...
  double WIDTH_CHECKBOX = 50;

  final double? COLUMN_FLEX = 1.0;
  ...
  Widget build(BuildContext context) {
    ...
    final bool anyRowSelectable = rows.any((MyDataRow row) => row.onSelectChanged != null);
    ...
    rows.where((MyDataRow row) => row.onSelectChanged != null) : <MyDataRow>[];
    ...
    if (displayCheckboxColumn) {
      tableColumns[0] = FixedColumnWidth(WIDTH_CHECKBOX);
    ...
    for (int dataColumnIndex = 0; dataColumnIndex < columns.length; dataColumnIndex += 1) {
      ...
      // } else {
      tableColumns[displayColumnIndex] = const IntrinsicColumnWidth(flex: 1.0);
      tableColumns[1] = FixedColumnWidth(WIDTH_CHECKBOX+30);
      // }
